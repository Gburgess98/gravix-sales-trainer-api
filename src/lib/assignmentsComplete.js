...
async function markCompletedById(opts) {
    ...
    if ((data || []).length > 0 && type) {
        ...
        for (const id of ids) {
            if (type === "sparring") {
                ...
                xpAwardedTotal += Number(xpRes?.xpAwarded || 0);
            }
            if (type === "call_review") {
                const xpRes = await awardXp({
                    repId,
    ...
    return { ok: true, updated, completedCount: updated, xpAwardedTotal };
}
...
export async function completeAssignmentsForTarget(opts) {
    const { repId, type, targetId, assignmentId = null, completedBy = "system", } = opts;
    if (!repId)
        return { ok: true, updated: 0, completedCount: 0, xpAwardedTotal: 0 };
    ...
    if (assignmentId) {
        ...
        return {
            ...
            completedCount: result.completedCount ?? result.updated,
            xpAwardedTotal: result.xpAwardedTotal ?? 0,
        };
    }
