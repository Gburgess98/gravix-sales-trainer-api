...
export function buildReviewFeedbackText(args) {
...
export function buildReviewFeedbackHtml(args) {
    const score = normaliseScore(args.scoreOverall);
    const voice = normaliseScore(args.voiceScore);
    const callUrl = `${getBaseUrl(args.appUrl)}/calls/${args.callId}`;
    const fillerWords = Array.isArray(args.fillerWords)
        ...
        ${flags.length ? `
          <div style="margin-top: 20px;">
            ...
            <div style="display: flex; gap: 8px; flex-wrap: wrap;">
              ${flags
        .map((flag) => `<span style="display:inline-block; padding:6px 10px; border-radius:999px; border:1px solid #3f3f46; background:#18181b; color:#e5e7eb; font-size:12px; font-weight:600;">${escapeHtml(flag)}</span>`)
        .join("")}
            </div>
          </div>
        ` : ""}
...
export async function sendReviewFeedbackEmail(args) {
