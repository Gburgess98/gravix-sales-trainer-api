...
import { auditUserCreated, writeAuditEvent, } from "../lib/audit";
...
function isRole(x) {
    return typeof x === "string" && ROLE_VALUES.includes(x);
}
...
----------------------------------------------------------------- */
adminRouter.get("/org-settings", requireManager, async (req, res) => {
    try {
...
----------------------------------------------------------------- */
adminRouter.patch("/org-settings", requireManager, async (req, res) => {
    try {
        const requester = String(req.header("x-user-id") || "").trim();
...
----------------------------------------------------------------- */
adminRouter.post("/users", requireManager, async (req, res) => {
    try {
        const { email, role, manager_id, office_id, } = req.body;
        if (!email) {
        ...
        if (error)
            throw error;
        await writeAuditEvent(supa, auditUserCreated(String(req.authUserId || requester || ""), data.id, data.company_id || null, data.office_id || null));
...
----------------------------------------------------------------- */
adminRouter.get("/users", requireManager, async (req, res) => {
    try {
        const supa = createClient(process.env.SUPABASE_URL, process.env.SUPABASE_SERVICE_ROLE_KEY);
        const requester = req.header("x-user-id");
        const { data: orgRow } = await supa
...
adminRouter.get("/status", async (_req, res) => {
    ...
    try {
        ...
        if (process.env.SUPABASE_URL && serviceKey) {
            ...
            push("supabase:buckets", !error, (buckets || []).map((b) => b.name));
        }
        else {
    ...
    try {
        ...
        if (slack) {
            ...
            push("slack:webhook_post", r?.ok === true || r?.status < 400, r?.status);
        }
        else {
...
adminRouter.post('/test-slack', async (req, res) => {
    try {
        ...
        if (!url)
            return res.status(400).json({ ok: false, error: 'SLACK_WEBHOOK_URL not set' });
        const text = req.body?.text || 'Gravix test: webhook is live ✅';
        const payload = {
            text,
            blocks: [
...
/* ----------------------------------------------------------------
   GET /v1/admin/usage
----------------------------------------------------------------- */
adminRouter.get("/usage", requireManager, async (req, res) => {
    try {
        const supa = createClient(process.env.SUPABASE_URL, process.env.SUPABASE_SERVICE_ROLE_KEY);
        const requester = req.header("x-user-id");
        const { data: orgRow } = await supa
...
adminRouter.post('/send-slack', async (req, res) => {
    try {
        const callId = req.query.callId?.trim();
        if (!callId)
            return res.status(400).json({ ok: false, error: 'missing_callId' });
        const webhook = (process.env.SLACK_WEBHOOK_URL || '').trim();
        if (!webhook)
            return res.status(400).json({ ok: false, error: 'no_webhook' });
...
adminRouter.post("/post-score-demo", async (_req, res) => {
    try {
        const slack = process.env.SLACK_WEBHOOK_URL;
        if (!slack)
            return res.status(400).json({ ok: false, error: "No SLACK_WEBHOOK_URL" });
        const payload = {
...
adminRouter.patch("/config", requireManager, async (req, res) => {
    try {
        const { streak_threshold, xp_multiplier, comeback_bonus, low_score_threshold, critical_score_threshold, } = req.body ?? {};
...
----------------------------------------------------------------- */
adminRouter.get("/reps", requireManager, async (_req, res) => {
    try {
        const url = process.env.SUPABASE_URL;
        const key = process.env.SUPABASE_SERVICE_ROLE_KEY || process.env.SUPABASE_SERVICE_KEY;
        if (!url || !key)
...
----------------------------------------------------------------- */
adminRouter.patch("/reps/:id", requireManager, async (req, res) => {
    try {
        const targetId = String(req.params.id || "").trim();
        const tier = req.body?.tier;
        if (!targetId)
            return res.status(400).json({ ok: false, error: "missing_rep_id" });
        if (!isRole(tier))
        ...
        if (requesterId && requesterId === targetId && !isManagerRole(tier)) {
            ...
            if (mgrErr)
                return res.status(500).json({ ok: false, error: mgrErr.message });
...
----------------------------------------------------------------- */
adminRouter.get("/persona-config", requireManager, async (req, res) => {
    try {
        const requester = String(req.header("x-user-id") || "").trim();
        const supa = createClient(process.env.SUPABASE_URL, process.env.SUPABASE_SERVICE_ROLE_KEY);
...
----------------------------------------------------------------- */
adminRouter.patch("/persona-config", requireManager, async (req, res) => {
    try {
        const requester = String(req.header("x-user-id") || "").trim();
        const body = (req.body || {});
        const supa = createClient(process.env.SUPABASE_URL, process.env.SUPABASE_SERVICE_ROLE_KEY);
        ...
        const mergedSettings = {
            ...existingSettings,
            buyer_style: typeof body.buyer_style === "string"
                ? body.buyer_style.trim()
                : existingSettings.buyer_style || null,
            industry_preset: typeof body.industry_preset === "string"
                ? body.industry_preset.trim()
                : existingSettings.industry_preset || null,
            objection_patterns: normaliseStringArray(body.objection_patterns),
            competitor_names: normaliseStringArray(body.competitor_names),
            common_pushbacks: normaliseStringArray(body.common_pushbacks),
            persona_memory: normaliseStringArray(body.persona_memory),
            emotional_tuning: {
                pressure_level: Number(body.emotional_tuning?.pressure_level ?? 50),
                trust_decay: Number(body.emotional_tuning?.trust_decay ?? 50),
                objection_aggression: Number(body.emotional_tuning?.objection_aggression ?? 50),
            },
        };
        ...
        await writeAuditEvent(supa, {
            ...
            metadata: {
                ...
                objection_count: mergedSettings.objection_patterns.length,
                memory_count: mergedSettings.persona_memory.length,
            },
        });
        return res.json({
            ok: true,
...
----------------------------------------------------------------- */
adminRouter.get("/partner/health", requirePartnerAdmin, async (req, res) => {
    try {
...
----------------------------------------------------------------- */
adminRouter.get("/super/health", requireSuperAdmin, async (req, res) => {
    try {
        const userId = String(req.header("x-user-id") || "").trim();
        const companies = await getVisibleCompanies(userId);
        return res.json({
...
----------------------------------------------------------------- */
adminRouter.get("/super/partners", requireSuperAdmin, async (req, res) => {
    try {
...
----------------------------------------------------------------- */
adminRouter.get("/partner/companies", requirePartnerAdmin, async (req, res) => {
    try {
        const userId = String(req.header("x-user-id") || "").trim();
        const supa = createClient(process.env.SUPABASE_URL, process.env.SUPABASE_SERVICE_ROLE_KEY, { auth: { autoRefreshToken: false, persistSession: false } });
        const companies = await getVisibleCompanies(userId);
        ...
        const partnerIds = [...new Set(companies.map((c) => c.partner_id).filter(Boolean))];
        ...
        const repCountMap = new Map();
        for (const row of (repRows || [])) {
            const cid = String(row.company_id ?? "");
            if (cid)
                repCountMap.set(cid, (repCountMap.get(cid) ?? 0) + 1);
        }
...
----------------------------------------------------------------- */
adminRouter.get("/partner/users", requirePartnerAdmin, async (req, res) => {
    try {
        const userId = String(req.header("x-user-id") || "").trim();
        const supa = createClient(process.env.SUPABASE_URL, process.env.SUPABASE_SERVICE_ROLE_KEY, { auth: { autoRefreshToken: false, persistSession: false } });
        const companies = await getVisibleCompanies(userId);
        ...
        const { data: partnerRows } = partnerIds.length
            ...
            : { data: [] };
        const partnerMap = new Map((partnerRows || []).map((p) => [p.id, p.name]));
        const companyPartnerMap = new Map(companies.map((c) => [c.id, c.partner_id ? (partnerMap.get(c.partner_id) ?? "") : ""]));
...
----------------------------------------------------------------- */
adminRouter.post("/super/impersonate", requireSuperAdmin, async (req, res) => {
    try {
        ...
        void logAuditEvent({
            ...
            entityType: "user",
            entityId: targetId,
            metadata: { targetName: target.name, targetTier: target.tier },
        });
        return res.json({
            ok: true,
            targetUserId: targetId,
...
----------------------------------------------------------------- */
adminRouter.post("/super/stop-impersonation", requireSuperAdmin, async (req, res) => {
    try {
        const actorId = String(req.header("x-user-id") || "").trim();
...
================================================================ */
async function assertUserEditScope(actorId, targetId, supa) {
    const { data: actor } = await supa.from("reps").select("tier, company_id").eq("id", actorId).maybeSingle();
    if (!actor)
        return { ok: false, status: 401, error: "actor_not_found" };
    const actorTier = String(actor.tier ?? "");
    const actorCo = String(actor.company_id ?? "");
    ...
    const targetCo = String(target.company_id ?? "");
    if (actorTier === "Manager") {
        if (!actorCo || actorCo !== targetCo)
...
adminRouter.get("/users/:id", async (req, res) => {
    try {
        ...
        if (!scope.ok)
            return res.status(scope.status).json({ ok: false, error: scope.error });
        const { data: rep, error } = await supa
            .from("reps")
        ...
        if (r.manager_id) {
            ...
            manager_name = mgr?.display_name ?? mgr?.name ?? null;
        }
    ...
    try {
        ...
        for (const { body: bodyField, db: dbField } of EDITABLE) {
            if (body[bodyField] === undefined)
                continue;
            let val = body[bodyField];
            if (dbField === "is_active")
                val = Boolean(val);
            if (typeof val === "string")
                val = val.trim();
            before[bodyField] = current?.[dbField] ?? null;
            after[bodyField] = val;
        ...
        if (patchErr)
            return res.status(500).json({ ok: false, error: patchErr.message });
        await logAuditEvent({
            actorUserId: actorId,
            targetUserId: targetId,
...
/* ================================================================
   ...
   Scope: Manager → own company; PartnerAdmin → partner companies;
          SuperAdmin → all.
================================================================ */
async function assertCompanyEditScope(actorId, targetCompanyId, supa) {
    const { data: actor } = await supa.from("reps").select("tier, company_id").eq("id", actorId).maybeSingle();
    if (!actor)
        return { ok: false, status: 401, error: "actor_not_found" };
    const actorTier = String(actor.tier ?? "");
    ...
    if (actorTier === "PartnerAdmin") {
        ...
        if (target?.partner_id !== partnerId)
            return { ok: false, status: 403, error: "forbidden_scope" };
        return { ok: true, actorTier };
    }
    return { ok: false, status: 403, error: "forbidden_scope" };
...
================================================================ */
adminRouter.get("/organisations", requireSuperAdmin, async (_req, res) => {
    try {
        const supa = createClient(process.env.SUPABASE_URL, process.env.SUPABASE_SERVICE_ROLE_KEY, { auth: { autoRefreshToken: false, persistSession: false } });
        ...
        for (const r of (repRows || [])) {
            ...
            entry.user_count += 1;
            if (r.company_id)
                entry.company_ids.add(String(r.company_id));
            if (r.created_at && r.created_at < entry.earliest)
        ...
        try {
            const { data: orgs } = await supa.from("organisations").select("id, name").in("id", orgIds);
            for (const o of (orgs || []))
                nameMap.set(String(o.id), String(o.name ?? o.id));
        }
        catch { /* table may not exist */ }
        // Fetch partner info via companies
        const allCompanyIds = Array.from(new Set((repRows || []).flatMap((r) => r.company_id ? [String(r.company_id)] : [])));
        ...
        if (allCompanyIds.length) {
            const { data: coRows } = await supa
                ...
                .in("id", allCompanyIds);
            const partnerIds = [...new Set((coRows || []).map((c) => c.partner_id).filter(Boolean))];
...
================================================================ */
adminRouter.get("/platform", requireSuperAdmin, async (_req, res) => {
    try {
        const supa = createClient(process.env.SUPABASE_URL, process.env.SUPABASE_SERVICE_ROLE_KEY, { auth: { autoRefreshToken: false, persistSession: false } });
        const [{ count: partner_count }, { count: company_count }, { count: user_count }, { count: audit_event_count }, { count: calls_processed },] = await Promise.all([
...
================================================================ */
adminRouter.get("/support/users", requireSuperAdmin, async (req, res) => {
    try {
...
adminRouter.get("/support/companies", requireSuperAdmin, async (req, res) => {
    try {
        const q = typeof req.query.q === "string" ? req.query.q.trim() : "";
        const limit = Math.min(Math.max(Number(req.query.limit ?? 50), 1), 200);
        const supa = createClient(process.env.SUPABASE_URL, process.env.SUPABASE_SERVICE_ROLE_KEY, { auth: { autoRefreshToken: false, persistSession: false } });
        ...
        const companyIds = (data || []).map((c) => c.id).filter(Boolean);
        ...
        if (companyIds.length) {
            ...
            for (const r of (repRows || [])) {
                if (r.company_id)
                    userCountMap.set(r.company_id, (userCountMap.get(r.company_id) ?? 0) + 1);
            }
        }
...
adminRouter.get("/support/impersonation-history", requireSuperAdmin, async (req, res) => {
    try {
        const limit = Math.min(Math.max(Number(req.query.limit ?? 50), 1), 200);
        const supa = createClient(process.env.SUPABASE_URL, process.env.SUPABASE_SERVICE_ROLE_KEY, { auth: { autoRefreshToken: false, persistSession: false } });
        const { data, error } = await supa
...
================================================================ */
adminRouter.get("/super/audit", requireSuperAdmin, async (req, res) => {
    try {
        const supa = createClient(process.env.SUPABASE_URL, process.env.SUPABASE_SERVICE_ROLE_KEY, { auth: { autoRefreshToken: false, persistSession: false } });
...
----------------------------------------------------------------- */
adminRouter.get("/partner/licences", requirePartnerAdmin, async (req, res) => {
    try {
        const supa = createClient(process.env.SUPABASE_URL, process.env.SUPABASE_SERVICE_ROLE_KEY, { auth: { autoRefreshToken: false, persistSession: false } });
        const userId = String(req.header("x-user-id") || "").trim();
        ...
        const companyIds = (allocations || []).map((a) => a.company_id);
        ...
        const companies = (allocations || []).map((a) => ({
            id: a.id,
        ...
        const totalAllocated = companies.reduce((sum, c) => sum + (c.allocated ?? 0), 0);
        const purchased = pool?.purchased ?? 0;
        return res.json({
            ok: true,
            ...
            pool: pool
                ? {
                    ...
                    purchased,
                    reserved: pool.reserved ?? 0,
                    renewed_at: pool.renewed_at ?? null,
...
----------------------------------------------------------------- */
adminRouter.get("/super/licences", requireSuperAdmin, async (req, res) => {
    try {
        const supa = createClient(process.env.SUPABASE_URL, process.env.SUPABASE_SERVICE_ROLE_KEY, { auth: { autoRefreshToken: false, persistSession: false } });
        // All partners
        const { data: partners, error: pErr } = await supa
        ...
        const allCompanyIds = [...new Set((allocations || []).map((a) => a.company_id))];
        let companyNameMap = new Map();
        ...
        const poolMap = new Map((pools || []).map((p) => [p.partner_id, p]));
        ...
        const result = (partners || []).map((p) => {
            ...
            const cos = (allocMap.get(p.id) || []).map((a) => ({
                id: a.id,
                company_id: a.company_id,
                company_name: companyNameMap.get(a.company_id) ?? null,
                allocated: a.allocated,
            ...
            const purchased = pool?.purchased ?? 0;
            const totalAllocated = cos.reduce((s, c) => s + (c.allocated ?? 0), 0);
            return {
                partner_id: p.id,
                partner_name: p.name,
                status: p.status,
...
----------------------------------------------------------------- */
adminRouter.get("/context/options", requirePartnerAdmin, async (req, res) => {
    try {
        const supa = createClient(process.env.SUPABASE_URL, process.env.SUPABASE_SERVICE_ROLE_KEY, { auth: { autoRefreshToken: false, persistSession: false } });
        const userId = String(req.header("x-user-id") || "").trim();
        ...
        const { data: rep } = await supa
            ...
            .eq("id", userId)
            .maybeSingle();
        const currentCompanyId = rep?.company_id ?? null;
        ...
        if (currentCompanyId) {
            const { data: co } = await supa
                ...
                .maybeSingle();
            currentPartnerId = co?.partner_id ?? null;
        }
        ...
        if (tier === "SuperAdmin") {
            ...
            const { data: ownPartner } = await supa
                ...
                .maybeSingle();
            if (ownPartner) {
                partners = [{ id: ownPartner.id, name: ownPartner.name }];
            }
        }
        return res.json({
            ok: true,
