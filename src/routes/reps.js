...
async function assertOrgScopeOr403(args) {
    ...
    const { data: requesterRow, error: requesterErr } = await sb
        ...
        .single();
    if (requesterErr) {
        throw new Error(`org_scope_lookup_failed:requester:${requesterErr.message}`);
    }
    if (!requesterRow) {
        throw new Error("org_scope_missing_requester");
    }
    if (String(requesterRow.org_id || "") !== orgId) {
    ...
    const { data: targetRow, error: targetErr } = await sb
        ...
        .single();
    if (targetErr) {
        throw new Error(`org_scope_lookup_failed:target:${targetErr.message}`);
    }
    if (!targetRow) {
        throw new Error("rep_not_found");
    }
    if (String(targetRow.org_id || "") !== orgId) {
        throw new Error("forbidden_org_scope");
    }
}
// GET /v1/reps/me — identity endpoint for the authenticated user
repsRouter.get("/me", async (req, res) => {
    try {
        ...
        const company_id = rep.company_id ?? null;
        let partner_id = null;
        if (company_id) {
...
repsRouter.get("/:id/overview", async (req, res) => {
    try {
        ...
        if (!requesterId)
            return res.status(401).json({ ok: false, error: "missing_requester" });
        if (!orgId)
            return res.status(400).json({ ok: false, error: "missing_org" });
        ...
        if (msg === "rep_not_found")
            return res.status(404).json({ ok: false, error: "rep_not_found" });
        if (msg.startsWith("org_scope_lookup_failed:"))
...
repsRouter.get("/:id/weakness-trends", async (req, res) => {
    try {
        const repId = String(req.params.id || "").trim();
        if (!repId || !UUID_RE.test(repId)) {
        ...
        const { data: calls, error: callsErr } = await sb
            .from("calls")
            .select("id,created_at,score_overall,intro_score,discovery_score,objection_score,close_score")
            .eq("user_id", repId)
        ...
        function calculateTrendDelta(values) {
            if (values.length < 2) {
                return 0;
            }
            ...
            const secondAvg = secondHalf.reduce((a, b) => a + b, 0) /
                Math.max(secondHalf.length, 1);
            return Math.round((secondAvg - firstAvg) * 10) / 10;
        }
        const introDelta = calculateTrendDelta(trends.intro.map((x) => x.value));
        const discoveryDelta = calculateTrendDelta(trends.discovery.map((x) => x.value));
        const objectionDelta = calculateTrendDelta(trends.objection_handling.map((x) => x.value));
        const closingDelta = calculateTrendDelta(trends.closing.map((x) => x.value));
        const overallDelta = calculateTrendDelta(trends.overall.map((x) => x.value));
        const momentumScore = Math.round((introDelta +
            discoveryDelta +
            objectionDelta +
            closingDelta +
            overallDelta) / 5);
        ...
        if (introDelta < -5) {
            regressionWarnings.push("Cold open performance declining");
        }
        if (objectionDelta < -5) {
            regressionWarnings.push("Objection handling confidence declining");
        }
        if (closingDelta < -5) {
            regressionWarnings.push("Closing performance weakening");
        }
        const aiSummary = momentumScore > 5
        ...
        const coachingCompletionTrend = {
            completed_assignments_estimate: safeCalls.filter((c) => Number(c.score_overall || 0) >= 70).length,
            struggling_sessions: safeCalls.filter((c) => Number(c.score_overall || 0) < 50).length,
        };
        return res.json({
            ok: true,
            rep_id: repId,
            ...
            coaching_completion_trend: coachingCompletionTrend,
        });
    }
    catch (err) {
        const msg = String(err?.message || err);
        console.error("GET /v1/reps/:id/weakness-trends error", msg);
        if (msg === "forbidden_org_scope") {
...
repsRouter.get("/:id/daily-feed", async (req, res) => {
    try {
        const repId = String(req.params.id || "").trim();
        if (!repId || !UUID_RE.test(repId)) {
            return res.status(400).json({
        ...
        const weakestArea = weakestEntry
            ...
            : null;
        const overallScores = safeCalls.map((c) => Number(c.score_overall || 0));
        ...
        const historicalAvg = overallScores.length
            ...
            : 0;
        const momentumDelta = Math.round((recentAvg - historicalAvg) * 10) / 10;
        const momentumInsight = momentumDelta > 5
        ...
        if (categoryScores.objection_handling < 55) {
            regressionWarnings.push("Objection handling confidence below target");
        }
        if (categoryScores.closing < 55) {
            regressionWarnings.push("Closing confidence weakening");
        }
        ...
        const coachingUrgency = weakestArea && weakestArea.score < 50
            ? "high"
            : weakestArea && weakestArea.score < 70
                ? "medium"
                : "low";
        const aiMotivationMessage = momentumDelta > 5
        ...
        const todaysFocus = weakestArea?.category === "objection_handling"
        ...
        const coachingSummary = `AI identified ${weakestArea?.category?.replace(/_/g, " ") ||
            "general performance"} as today's main coaching focus.`;
        return res.json({
            ok: true,
            rep_id: repId,
        ...
        const msg = String(err?.message || err);
        console.error("GET /v1/reps/:id/daily-feed error", msg);
        if (msg === "forbidden_org_scope") {
            return res.status(403).json({
                ok: false,
                error: "forbidden_org_scope",
...
repsRouter.get("/:id/xp", async (req, res) => {
