...
function buildAccountVisibilityFilter(query, requester) {
      ...
      `)
        ...
        const enrichedAccounts = await Promise.all((accounts || []).map(async (account) => {
            ...
            return {
                ...
                latest_activity_at: callsRes.data?.[0]?.created_at ||
                    account.updated_at ||
                    account.created_at,
            };
        }));
...
async function resolveCompanyId(userId) {
    ...
    if (userRow?.company_id)
        return String(userRow.company_id);
    ...
    const { data: repRow } = await supa
        ...
        .eq('id', userId)
        .maybeSingle();
    return repRow?.company_id ? String(repRow.company_id) : null;
}
/* ----------------------------------------------------------------
   POST /v1/accounts
        ...
        return res.json({
            ...
            account: {
                ...
                stats: {
                    ...
                    avg_score: calls.length > 0
                        ? Math.round(calls.reduce((acc, c) => acc + Number(c.score || 0), 0) / calls.length)
                        : null,
                },
            },
            linked_contacts: contactsRes.data || [],
            linked_calls: calls,
            timeline,
        });
    }
...
----------------------------------------------------------------- */
router.get('/:id/intelligence-timeline', async (req, res) => {
    try {
        const userId = getUserId(req);
        const requester = await getRequesterContext(userId);
        if (!requester?.company_id) {
      ...
      `)
        ...
        const [callsRes, tasksRes, escalationsRes, summariesRes] = await Promise.all([
            supa
                .from('calls')
                .select(`
            id,
        ...
        if (escalationsRes.error)
            throw escalationsRes.error;
        if (summariesRes.error)
            throw summariesRes.error;
        const callEvents = (callsRes.data || []).map((call) => ({
            id: `call:${call.id}`,
            source_id: call.id,
            type: 'call',
            severity: Number(call.score || 0) < 60
                ? 'warning'
                : 'info',
            title: call.title || 'Sales call logged',
            description: Number(call.score || 0) > 0
        ...
        const taskEvents = (tasksRes.data || []).map((task) => ({
            ...
            source_id: task.id,
            type: 'ai_task',
            severity: task.urgency === 'critical'
                ? 'critical'
                : task.urgency === 'high'
                    ? 'warning'
                    : 'info',
        ...
        const escalationEvents = (escalationsRes.data || []).map((escalation) => ({
            ...
            description: escalation.escalation_reason ||
        ...
        const summaryEvents = (summariesRes.data || []).map((summary) => ({
            id: `summary:${summary.id}`,
            source_id: summary.id,
            type: 'ai_summary',
            severity: Number(summary.churn_risk || 0) >= 70
                ? 'critical'
        ...
        const timeline = [
            ...
            .sort((a, b) => new Date(b.occurred_at).getTime() -
            new Date(a.occurred_at).getTime());
        const criticalCount = timeline.filter((event) => event.severity === 'critical').length;
        const warningCount = timeline.filter((event) => event.severity === 'warning').length;
        return res.json({
            ok: true,
            account,
            timeline,
...
router.post('/:id/tasks/generate', async (req, res) => {
    try {
      ...
      `)
        ...
        const avgScore = callList.length > 0
            ? Math.round(callList.reduce((acc, c) => acc + Number(c.score || 0), 0) / callList.length)
            : 0;
        ...
        if (!account.owner_id) {
            generatedTasks.push({
                ...
                description: 'This account currently has no accountable owner assigned.',
                category: 'ownership',
                urgency: 'high',
                status: 'open',
        ...
        if (avgScore < 60) {
            generatedTasks.push({
                ...
                description: 'Recent account call performance has deteriorated below acceptable thresholds.',
                category: 'coaching',
                urgency: 'critical',
                status: 'open',
                assigned_to: account.owner_id || requester.id,
        ...
        if (callList.length === 0) {
            generatedTasks.push({
                ...
                description: 'No recent sales activity detected for this account.',
                category: 'engagement',
                urgency: 'high',
                status: 'open',
                assigned_to: account.owner_id || requester.id,
        ...
        const insertPayload = generatedTasks.map((task) => ({
            ...task,
            due_at: new Date(now + 1000 * 60 * 60 * 24 * 3).toISOString(),
...
router.post('/:id/summary', async (req, res) => {
    try {
      ...
      `)
        ...
        const healthStatus = String(req.body?.health_status || 'stable').trim();
        const churnRisk = Number(req.body?.churn_risk || 0);
        const nextBestAction = String(req.body?.next_best_action || '').trim();
        const managerNotes = String(req.body?.manager_notes || '').trim();
        const payload = {
            account_id: account.id,
            summary,
...
----------------------------------------------------------------- */
router.get('/:id/coaching-actions', async (req, res) => {
    try {
        const userId = getUserId(req);
        const requester = await getRequesterContext(userId);
        if (!requester?.company_id) {
      ...
      `)
        ...
        return res.status(500).json({
            ok: false,
            error: e?.message || 'account_coaching_actions_fetch_failed',
        });
    }
});
/* ----------------------------------------------------------------
...
----------------------------------------------------------------- */
router.post('/:id/coaching-action', async (req, res) => {
    try {
        const userId = getUserId(req);
        const requester = await getRequesterContext(userId);
        if (!requester?.company_id) {
      ...
      `)
        ...
        const actionType = String(req.body?.action_type || 'coaching').trim();
        const urgency = String(req.body?.urgency || 'medium').trim();
        const title = String(req.body?.title || 'AI coaching action').trim();
        ...
        const { data: coachingAction, error: coachingError } = await supa
            .from('account_coaching_actions')
            .insert(payload)
            .select(`
          id,
        ...
        return res.status(500).json({
            ok: false,
            error: e?.message || 'account_coaching_action_failed',
        });
    }
});
/* ----------------------------------------------------------------
...
router.get('/escalations', async (req, res) => {
    try {
      ...
      `)
            ...
            .order('created_at', { ascending: false })
            .limit(100);
        const { data: escalations, error } = await escalationQuery;
        if (error) {
            throw error;
        }
        return res.json({
        ...
        return res.status(500).json({
            ok: false,
            error: e?.message || 'account_escalations_fetch_failed',
        });
    }
});
/* ----------------------------------------------------------------
...
router.post('/:id/escalate', async (req, res) => {
    try {
      ...
      `)
        ...
        const escalationReason = String(req.body?.escalation_reason ||
            'AI detected elevated account risk.').trim();
        const workflowStage = String(req.body?.workflow_stage || 'intervention_required').trim();
        const interventionRequired = req.body?.intervention_required !== false;
        const assignedManagerId = String(req.body?.assigned_manager_id ||
        ...
        const { data: escalation, error: escalationError } = await supa
            .from('account_escalations')
            .insert(payload)
            .select(`
          id,
...
----------------------------------------------------------------- */
router.get('/:id/rescue-engine', async (req, res) => {
    try {
        const userId = getUserId(req);
        const requester = await getRequesterContext(userId);
        if (!requester?.company_id) {
      ...
      `)
        ...
        const recentAvg = recentCalls.length > 0
            ? Math.round(recentCalls.reduce((acc, c) => acc + Number(c.score || 0), 0) / recentCalls.length)
            : 0;
        const hasOwner = !!account.owner_id;
        const noRecentActivity = calls.length === 0 ||
            (() => {
        ...
        const riskBand = churnRiskScore >= 75
            ? 'critical'
            : churnRiskScore >= 55
                ? 'high'
                : churnRiskScore >= 35
                    ? 'medium'
                    : 'low';
        const rescueRecommendations = [];
        if (!hasOwner) {
            rescueRecommendations.push('Assign an accountable rep or manager immediately.');
        }
        if (avgScore < 60) {
            rescueRecommendations.push('Review recent failed calls and run replay coaching.');
        }
        if (recentAvg < avgScore) {
            rescueRecommendations.push('Recent performance is declining. Trigger manager intervention.');
        }
        if (noRecentActivity) {
            rescueRecommendations.push('No recent account activity detected. Schedule immediate outreach.');
        }
        const nextBestAction = !hasOwner
            ...
            route: riskBand === 'critical'
                ...
                : riskBand === 'high'
                    ...
                    : hasOwner
                        ? 'assigned_owner'
                        : 'manager_queue',
            escalation_required: riskBand === 'critical' ||
                riskBand === 'high',
        };
        return res.json({
            ok: true,
            ...
            risk_band: riskBand,
            ai_rescue_recommendations: rescueRecommendations,
            next_best_action: nextBestAction,
            manager_intervention_suggestions: managerInterventionSuggestions,
            automated_escalation: escalationRouting,
            account_save_workflow: saveWorkflow,
            ai_summary: riskBand === 'critical'
