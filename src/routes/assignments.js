...
function getFailureMultiplier(failureCount) {
    if (!failureCount || failureCount <= 1)
        return 1;
    if (failureCount === 2)
        return 1.3; // repeated
    if (failureCount === 3)
        return 1.7; // concerning
    if (failureCount >= 4)
        return 2.2; // 🔥 major issue
    return 1;
}
function calculateWeaknessPriority(args) {
    ...
    const businessImpactMultiplier = args.section === "close" ? 1.5 :
        args.section === "objection" ? 1.2 :
            1;
    const failureMultiplier = getFailureMultiplier(args.failureCount || 1);
    return (((sectionScore * severityScore * businessImpactMultiplier) * failureMultiplier) +
        (frequencyScore * 2) +
        recencyScore);
}
function formatDayMon(d) {
    try {
...
function safeTypeNudgeLabel(t) {
    const type = String(t || "").toLowerCase();
    if (type === "sparring")
        return "sparring drill";
    if (type === "call_review")
        return "call review";
    return "task";
}
// 🔥 DRILL MEMORY AGGREGATION
async function getRepSectionMemory(supa, repId) {
...
async function getRepWeaknessRanking(supa, repId) {
    const { data, error } = await supa
        .from("crm_activities")
        .select("meta, created_at")
        .eq("type", "review_flag")
...
async function getUserContext(supa, userId) {
    const { data } = await supa
        .from("users")
...
// 🔥 IMPROVEMENT SCORE
function getImprovementScore(sectionMemory) {
    if (!sectionMemory)
        return 0;
...
function getNextDifficulty(sectionMemory) {
...
async function sendSlackWebhook(text) {
    ...
    if (!url)
        throw new Error("missing_slack_webhook_url");
...
export function assignmentsRoutes() {
    const r = Router();
    const SELECT_FIELDS = "id,rep_id,manager_id,type,target_id,title,status,due_at,created_at,completed_at,completed_by,source,meta";
    function normalizeAssignment(row) {
        const meta = row && typeof row.meta === "object" && row.meta ? row.meta : null;
        const thresholdBand = typeof row?.threshold_band === "string"
            ? String(row.threshold_band)
            : typeof meta?.threshold_band === "string"
                ? String(meta.threshold_band)
                : null;
        const needsManagerReview = typeof row?.needs_manager_review === "boolean"
            ? Boolean(row.needs_manager_review)
            : Boolean(meta?.needs_manager_review);
        const flaggedCall = typeof row?.flagged_call === "boolean"
        ...
        return {
            ...
            assignment_origin: typeof meta?.assignment_origin === "string"
                ? String(meta.assignment_origin)
                : null,
            flag_section: typeof meta?.flag_section === "string"
                ? String(meta.flag_section)
                : null,
            score_before: typeof meta?.score_before === "number"
                ? meta.score_before
                : null,
        };
    }
    function sortOpenAssignments(rows) {
    ...
    function buildAssignmentsSummary(rows) {
        const now = Date.now();
        ...
        const flagged = normalized.filter((r) => Boolean(r.flagged_call) || Boolean(r.threshold_band) || Boolean(r.needs_manager_review));
        const critical = normalized.filter((r) => r.threshold_band === "critical" || Boolean(r.needs_manager_review));
    ...
    r.get("/by-target", async (req, res) => {
        try {
            ...
            const type = String(req.query.type || "call_review").trim().toLowerCase() || "call_review";
    ...
    r.get("/", async (req, res) => {
        ...
        const repIdFromQuery = String(req.query.repId || "").trim();
        ...
        let q = supa
            ...
            .order("created_at", { ascending: false });
        if (repId !== userId) {
            const managerId = String(req.authUserId || "").trim();
    ...
    r.post("/", requireManager, async (req, res) => {
        const supa = getSupaAdmin();
        const managerId = String(req.authUserId || "");
        if (!managerId) {
            return res.status(401).json({ ok: false, error: "missing_user" });
        }
        const { rep_id, type, target_id, target, title, due_at, source, notes, meta, } = req.body ?? {};
        ...
        const cleanNotes = typeof notes === "string"
            ...
            : null;
        const cleanTarget = typeof target === "string"
            ? target.trim().slice(0, 1000)
            : null;
        // 🧠 ADVANCED MEMORY-BASED DIFFICULTY
        let difficulty = "easy";
        let sectionMemory = null;
        try {
            ...
            rankedWeaknesses = [
                {
                    ...
                    severity: safeMeta?.threshold_band === "critical" ? "critical" : "low",
        ...
        const drillType = sectionForDrill === "objection"
            ? "objection_handling"
        ...
        const alreadyExists = (existing || []).some((a) => {
            const m = a.meta || {};
            return (m?.flag_section === safeMeta?.flag_section &&
        ...
        const payload = {
            ...
            meta: {
                ...
                // 🧠 MEMORY TRACKING
                failure_count: (safeMeta?.failure_count ?? 1) +
                ...
                replay_context: type === "replay"
                    ? {
                        replay_call_id: cleanTarget,
                        replay_reason: safeMeta?.replay_reason ||
                            "Replay failed sales moment",
                    }
                    : null,
                drill_context: type === "drill"
                    ? {
                        drill_name: cleanTarget,
                        drill_focus: safeMeta?.flag_section || "general",
    ...
    r.patch("/:id/complete", async (req, res) => {
        ...
        const alreadyCompleted = String(row.status || "") === "completed" ||
        ...
        if (alreadyCompleted) {
            const { data: current, error: curErr } = await supa
                ...
                .maybeSingle();
            if (curErr)
                return res.status(500).json({ ok: false, error: curErr.message });
            if (!current)
                return res.status(404).json({ ok: false, error: "not_found" });
        ...
        try {
            const section = data?.meta?.flag_section;
            if (section) {
                await supa.from("crm_activities").insert({
                    type: "drill_memory",
                    rep_id: userId,
    ...
    r.post("/:id/nudge", requireManager, async (req, res) => {
        try {
            const supa = getSupaAdmin();
            const managerId = String(req.authUserId || "").trim();
            if (!managerId)
            ...
            if (String(a.manager_id || "") !== managerId) {
                return res.status(403).json({ ok: false, error: "forbidden_not_owner" });
            }
            ...
            const dueAtIso = a.due_at ? String(a.due_at) : "";
            ...
            try {
                const { data: rep, error: repErr } = await supa
                    ...
                    .maybeSingle();
                if (!repErr && rep)
                    repName = String(rep.name || "").trim() || null;
            }
            catch {
                // ignore
            }
            ...
            try {
                ...
                if (!rowsErr && Array.isArray(rows)) {
                    const openCount = rows.filter((r) => String(r.status || "").toLowerCase() !== "completed").length;
                    if (openCount >= 2) {
    ...
    r.patch("/manager/:id", async (req, res) => {
        try {
            const id = String(req.params.id || "").trim();
            if (!id)
                return res.status(400).json({ ok: false, error: "missing_id" });
            ...
            if (!current)
                return res.status(404).json({ ok: false, error: "not_found" });
            if (String(current.manager_id || "") !== String(managerId)) {
    ...
    r.delete("/:id", requireManager, async (req, res) => {
        const supa = getSupaAdmin();
        const managerId = String(req.authUserId || "");
        const id = String(req.params.id || "").trim();
        if (!id)
        ...
        if (!row)
            return res.status(404).json({ ok: false, error: "not_found" });
        if (!managerId || String(row.manager_id || "") !== managerId) {
            return res.status(403).json({ ok: false, error: "forbidden_not_owner" });
        }
    ...
    r.get("/manager", requireManager, async (req, res) => {
        const supa = getSupaAdmin();
        const managerId = String(req.authUserId || "");
    ...
    r.get("/manager/signals", requireManager, async (req, res) => {
        const supa = getSupaAdmin();
        const managerId = String(req.authUserId || "");
        const now = new Date();
        const nowIso = now.toISOString();
        ...
        const completed_7d = created7d.filter((a) => String(a.status || "") === "completed" &&
            String(a.completed_by || "").toLowerCase() !== "manager").length;
        const manager_overrides_7d = created7d.filter((a) => String(a.status || "") === "completed" &&
            String(a.completed_by || "").toLowerCase() === "manager").length;
        ...
        const completedByRepLast7d = new Set(list
        ...
        let stale_reps_7d = staleRepIds.map((id) => ({
            rep_id: id,
            name: null,
            tier: null,
        }));
    ...
    r.get("/manager/trust", requireManager, async (req, res) => {
        try {
            const supa = getSupaAdmin();
            const managerId = String(req.authUserId || "");
            if (!managerId)
            ...
            const created7 = list;
            const completed24 = list.filter((a) => inRange(a.completed_at, since24h) && String(a.completed_by || "").toLowerCase() !== "manager");
            const completed7 = list.filter((a) => inRange(a.completed_at, since7d) && String(a.completed_by || "").toLowerCase() !== "manager");
            const overrides24 = list.filter((a) => inRange(a.completed_at, since24h) && String(a.completed_by || "").toLowerCase() === "manager");
            const overrides7 = list.filter((a) => inRange(a.completed_at, since7d) && String(a.completed_by || "").toLowerCase() === "manager");
            const isAuto = (a) => String(a.completed_by || "").toLowerCase() === "system";
            const auto24 = completed24.filter(isAuto);
            const auto7 = completed7.filter(isAuto);
            const pct = (done, created) => {
                if (!created)
                    return 0;
            ...
            const reasonFor = (a) => {
                ...
                if (type === "custom" && isPast(a.due_at))
                    return "Overdue custom task";
                if (type === "sparring")
                    return "Sparring not completed";
                if (type === "call_review" && !hasTarget)
                    return "No call picked";
                if (type === "call_review" && hasTarget)
                    return "Call review not completed";
                if (isPast(a.due_at))
                    return "Overdue";
            ...
            const topStuckReason = Object.entries(reasonCounts)
                .sort((a, b) => b[1] - a[1])
            ...
            for (const a of list) {
                ...
                if (String(a.status || "").toLowerCase() === "assigned") {
                    repAgg[repId].open += 1;
                    if (isPast(a.due_at))
                        repAgg[repId].overdue += 1;
                }
                if (a.completed_at) {
                    if (isToday(a.completed_at))
                        repAgg[repId].completed_today += 1;
                    if (!repAgg[repId].last_completed_at)
                        repAgg[repId].last_completed_at = String(a.completed_at);
                    else {
        ...
        try {
            ...
            for (const r of rows) {
                ...
                if (String(r.status).toLowerCase() === "completed")
                    ...
                    item.open++;
                if (r.due_at && new Date(r.due_at).getTime() < Date.now())
                    item.overdue++;
                if (r.flagged_call || r.threshold_band || r.needs_manager_review)
                    item.flagged++;
                if (r.threshold_band === "critical" || r.needs_manager_review)
                    item.critical++;
                if (String(r.source || "").toLowerCase() === "flagged_call_auto")
                    item.auto_created++;
            }
    ...
    r.get("/me", async (req, res) => {
        try {
            ...
            const { data: rep, error: repErr } = await supa
                ...
                .maybeSingle();
            if (repErr)
                return res.status(500).json({ ok: false, error: repErr.message });
            const xp_total = Number(rep?.xp) || 0;
            ...
            try {
                ...
                if (!evErr && Array.isArray(events)) {
                    xp_today = events.reduce((sum, e) => sum + (Number(e?.xp) || 0), 0);
                }
            }
            catch {
                // ignore
