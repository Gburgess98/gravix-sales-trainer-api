...
function uid(req) { const v = (req.header("x-user-id") || "").trim(); if (!v)
    throw new Error("Missing x-user-id"); return v; }
r.post("/assign", async (req, res) => {
    try {
        const body = Create.parse(req.body);
        const requester = uid(req);
        let activityDebug = { attempted: false, ok: false, error: null };
        ...
        const reviewFlags = Array.isArray(call?.analysis_json?.review_flags)
            ? call.analysis_json.review_flags
            : [];
        const thresholdBand = typeof call?.analysis_json?.threshold_band === "string"
            ? String(call.analysis_json.threshold_band)
            : null;
        const needsManagerReview = Boolean(call?.analysis_json?.needs_manager_review);
        ...
        if (duplicateCheck.error) {
            const msg = String(duplicateCheck.error?.message ?? "").toLowerCase();
        ...
        if (!duplicateCheck.error && duplicateCheck.data) {
            console.log("[coach/assign] skipped duplicate open assignment", {
                existingId: duplicateCheck.data?.id ?? null,
                call_id: body.callId,
            ...
            return res.json({
                ...
                reporting: {
                    assignment_origin: String(duplicateCheck.data?.source || assignmentSource),
                    flagged_call: Boolean(duplicateCheck.data?.meta?.flagged_call ?? isFlaggedCall),
                    threshold_band: duplicateCheck.data?.meta?.threshold_band ?? thresholdBand,
                    review_flag_count: Array.isArray(duplicateCheck.data?.meta?.review_flags)
                        ? duplicateCheck.data.meta.review_flags.length
                        : reviewFlags.length,
                    needs_manager_review: Boolean(duplicateCheck.data?.meta?.needs_manager_review ?? needsManagerReview),
                },
                activity_debug: {
                    attempted: false,
                    ok: true,
        ...
        const assignmentPayload = {
            ...
            meta: {
                ...
                score_overall: call?.score_overall ?? null,
                // NEW: tracking fields for analytics
                flag_sections: reviewFlags.map((f) => f.section).filter(Boolean),
                score_before: call?.score_overall ?? null,
            },
        };
        ...
        try {
            const activityPayload = {
                ...
                meta: {
                    coach_assignment_id: data?.id ?? null,
                    drill_id: body.drillId,
                    requester,
                    assignee_user_id: body.assigneeUserId,
                    notes: body.notes ?? null,
            ...
            if (insertRes.error) {
                ...
                activityDebug.error = String(insertRes.error?.message ?? insertRes.error);
        ...
        console.log("[coach/assign] created", {
            coachAssignmentId: data?.id ?? null,
            call_id: body.callId,
            org_id: call?.org_id ?? null,
            rep_id: body.assigneeUserId,
