...
async function getInternalUser(userId: string): Promise<InternalUser | null> {
...
async function requireInternal(req: any, res: any, next: any) {
  try {
...
router.get("/health", requireInternal, async (req: any, res) => {
  res.json({
    ok: true,
    internal: true,
...
router.post("/impersonate", requireInternal, async (req: any, res) => {
  const internalUser = req.internalUser as InternalUser;
  ...
  const context = buildImpersonationContext(
    internalUser.id,
    targetUserId
  );
