...
import { getPersonaConfig, buildPersonaBehaviourSummary, } from "../personas";
// TIER 2A Day 101 — Sparring Brain: pure state manager + provider router
import { createInitialSparringState, updateSparringState, coerceState, generateBuyerReply, withStateDirectives, } from "../sparring";
...
const STREAK_MIN_SCORE = Number.isFinite(Number(process.env.SPAR_STREAK_MIN_SCORE))
    ...
    : 75;
const STREAK_USE_CALIBRATION = String(process.env.SPAR_STREAK_USE_CALIBRATION || "1").trim() !== "0";
...
function isGoodTurn(rawTurnScore) {
    // "Good" is judged on the calibrated streak score (Option A)
    return getStreakScore(rawTurnScore) >= STREAK_MIN_SCORE;
}
function updateStreakMeta(prev, rawTurnScore, opts) {
    const meta = { ...(prev || {}) };
    // Hard defaults so meta fields are never null/undefined in DB/UI
    if (typeof meta.streak !== "number")
        meta.streak = 0;
    if (typeof meta.best_streak !== "number")
        meta.best_streak = 0;
    if (typeof meta.xp_multiplier !== "number")
        meta.xp_multiplier = streakMultiplier(Number(meta.streak || 0), opts?.streakThreshold ?? 3);
    if (typeof meta.last_turn_score !== "number")
        meta.last_turn_score = 0;
    if (typeof meta.last_turn_score_raw !== "number")
        meta.last_turn_score_raw = 0;
    if (typeof meta.comeback_pending !== "boolean")
        meta.comeback_pending = false;
    if (typeof meta.xp_bonus_pending !== "number")
        meta.xp_bonus_pending = 0;
...
function scoreRepTurnHeuristic(repText, buyerText) {
    ...
    if (buyerPriceObj && valueLanguage)
        coach_note = "Good: validate price concern then anchor ROI/payback with a number.";
    else if (buyerPriceObj && !valueLanguage)
        coach_note = "Anchor ROI before discussing cost. Tie it to their world (time/money).";
    else if (!askedQuestion)
        coach_note = "Ask a sharper discovery question to regain control.";
    else if (closeLanguage)
        coach_note = "Nice: you moved toward a next step. Keep the close binary.";
    return {
        turn_score: total,
        micro_breakdown: {
            opener,
...
function getDifficultyFromRow(row) {
    if (!row)
        return "normal";
...
function getModeFromRow(row) {
    if (!row)
        return "standard";
    return row.meta?.mode || "standard";
}
function computeOutcomeFromScore(total) {
...
function computeXpForScore(total, difficulty, mode) {
    const difficultyBaseXp = {
        easy: 20,
        normal: 30,
        hard: 45,
        nightmare: 60,
...
function getPersonaMutationState(personaId, difficulty) {
    const base = {
        volatility: 40,
        resistance: 40,
        unpredictability: 30,
    ...
    // 🔥 Difficulty scaling
    const difficultyBoost = difficulty === "nightmare"
        ? 30
        : difficulty === "hard"
            ? 20
            : 0;
...
function clampEmotion(n) {
    if (!Number.isFinite(n))
        return 0;
    return Math.max(0, Math.min(100, Math.round(n)));
}
function getInitialEmotionalState(personaId, difficulty) {
    const base = { anger: 10, boredom: 10, trust: 30 };
    switch (personaId) {
        case "angry":
            return {
...
function evaluateRepWeakness(text) {
...
function applyEmotionalDelta(prev, opts) {
...
function getNextStackedObjection(opts) {
    ...
    // 🔥 CONDITIONS TO STACK
    const shouldStack = turnsSoFar > 4 &&
        (emotional.anger > 50 ||
...
function getUserIdHeader(req) {
    const raw = req.headers['x-user-id']?.toString().trim();
    if (!raw) {
...
function getOrgIdFromRequest(req) {
    const headerOrg = req.headers['x-org-id']?.toString().trim();
    if (headerOrg)
        return headerOrg;
    const bodyOrg = typeof req.body?.orgId === 'string'
...
async function loadTeamSettingsSnapshot(orgId) {
    if (!orgId) {
        return {
            org_id: null,
    ...
    for (const sel of selectCandidates) {
        ...
        throw new Error(r.error?.message ?? 'team_settings_snapshot_failed');
    }
    return {
        org_id: orgId,
    ...
    try {
      ...
      `)
        ...
        return {
            ...
            buyer_style: typeof settings.buyer_style === "string"
                ? settings.buyer_style
                : null,
            industry_preset: typeof settings.industry_preset === "string"
                ? settings.industry_preset
                : null,
            emotional_tuning: settings.emotional_tuning &&
...
function computeAdaptiveDifficulty(opts) {
    ...
    const aggression = Number(opts.emotionalTuning?.objection_aggression ?? 50);
...
function buildBehaviourDiagnostics(opts) {
    ...
    const escalationScore = Math.min(100, Math.round(objections.length * 12 +
        anger * 0.35 +
        boredom * 0.2));
    const recoveryDetected = timeline.some((x) => {
...
function buildCoachingInsights(opts) {
    ...
    if (diagnostics.trustCollapseDetected) {
        insights.push("Buyer trust collapsed during the conversation. Rep likely failed to anchor value or build confidence.");
    }
    if (diagnostics.angerSpikeDetected) {
        insights.push("Buyer frustration escalated aggressively. Rep may have pushed too hard or failed to handle objections calmly.");
    }
    if (diagnostics.disengagedDetected) {
        insights.push("Buyer disengagement detected. Rep likely lost conversational control or failed discovery.");
    }
    if (diagnostics.recoveryDetected) {
        insights.push("Rep successfully recovered buyer trust at certain points in the conversation.");
    }
    const objections = Array.isArray(opts.objectionHistory)
        ? opts.objectionHistory
        : [];
    if (objections.length >= 3) {
        insights.push("Multiple objections stacked during the call. Rep struggled under sustained pressure.");
    }
    const failed = Array.isArray(opts.failedMoments)
        ? opts.failedMoments
        : [];
    if (failed.length >= 2) {
        insights.push("Repeated weak moments detected. Recommend replay drills focused on objection handling and confidence control.");
    }
    return insights;
}
function buildReplayComparison(opts) {
    ...
    const originalScore = Number(replay?.original_score ||
        replay?.source_score ||
        0);
    const scoreDelta = replayScore && originalScore
        ? replayScore - originalScore
        : null;
    const trustRecoveryDelta = trust - 25;
    const angerReductionDelta = 75 - anger;
    const objectionImprovementDelta = Math.max(0, 100 - ((opts.objectionHistory || []).length * 12));
    const replayImprovementScore = Math.max(0, Math.min(100, Math.round((scoreDelta || 0) * 1.4 +
        trustRecoveryDelta * 0.45 +
        angerReductionDelta * 0.25 +
        objectionImprovementDelta * 0.2)));
    return {
        original_session_id: replay?.source_session_id || null,
        replay_attempt: Number(replay?.replay_attempt || 0),
        original_score: originalScore || null,
        replay_score: replayScore || null,
        score_delta: scoreDelta,
        trust_recovery_delta: trustRecoveryDelta,
        anger_reduction_delta: angerReductionDelta,
        objection_improvement_delta: objectionImprovementDelta,
        replay_improvement_score: replayImprovementScore,
        confidence_recovery_detected: trust >= 60 && anger <= 40,
    };
}
...
`;
    ...
    const difficulty = opts.difficulty ||
        persona?.difficultyDefault ||
        persona?.difficulty_default ||
        "normal";
    const personaLabel = persona?.label || "Generic Buyer";
    ...
    try {
        ...
        console.warn("[sparring] buildPersonaBehaviourSummary failed, falling back to basic traits", e);
    }
    const lines = [];
    ...
    const modeLine = mode === "time_trial"
    ...
    const diffLine = difficulty === "nightmare"
        ...
        return res.json({
            ...
            analytics: {
                ...
                coaching_progression: {
                    replay_attempt: replayComparison.replay_attempt,
                    confidence_recovery_detected: replayComparison.confidence_recovery_detected,
                    improvement_detected: (replayComparison.score_delta || 0) > 0,
                    trust_recovery_delta: replayComparison.trust_recovery_delta,
                    replay_improvement_score: replayComparison.replay_improvement_score,
                },
                analytics_state: meta.analytics_state || {},
                replay_state: meta.replay_state || null,
                company_profile: meta.company_persona_profile || null,
            },
        });
    }
    catch (e) {
        ...
        personas: [
            {
                ...
                description: "Pushes back on price early and often. Fixated on ROI and alternatives.",
            ...
            },
            {
                ...
                description: "Easily irritated, talks over you, demanding strong justification.",
            ...
            },
            {
                ...
                description: "Barely gives anything. You must carry the conversation and create momentum.",
            ...
            },
            {
                ...
                description: "Wants numbers, risk minimisation, and clear upside justification.",
                difficulty_default: "hard",
            },
            {
                id: "procurement",
                ...
                description: "Everything must go through a committee or existing vendor. Loves process.",
                difficulty_default: "normal",
            },
        ],
    });
...
router.get("/leaderboard/:personaId", async (req, res) => {
    try {
        ...
        for (const row of rows) {
            const score = typeof row.total_score === "number"
                ? row.total_score
            ...
            if (typeof score !== "number" || Number.isNaN(score))
                continue;
            const diffKey = normaliseDifficulty(row.difficulty || row.meta?.difficulty || null);
            total += 1;
...
router.post('/log', express.json(), async (req, res) => {
    ...
    const rep_id = typeof repId === 'string' && repId.length ? repId : null;
    const persona_id = (typeof personaId === 'string' && personaId.trim().length) ? personaId.trim() : 'unknown';
    ...
    if (rep_id) {
        try {
            ...
            if (error)
                warnings.push(`xp_events insert: ${error.message}`);
        }
        catch (e) {
    ...
    if (orgId && process.env.SPAR_ACTIVITY_LOG === '1') {
        try {
            ...
            if (error)
                warnings.push(`activities insert: ${error.message}`);
        }
        catch (e) {
...
router.post("/score", express.json(), async (req, res) => {
    ...
    try {
        ...
        if (typeof sessionId === "string" && sessionId.trim().length) {
            ;
            global.__last_completed_count = 0;
            ;
            global.__last_xp_awarded_total = 0;
            const { data: row, error: selErr } = await supa
                .from("sparring_sessions")
                .select("id, meta, difficulty, total_score, xp_awarded, duration_ms, turns, summary, flags")
                .eq("id", sessionId)
                .single();
            if (selErr || !row) {
                return res.status(400).json({
            ...
            else if (typeof row.total_score === "number") {
                numericTotal = clampScore(row.total_score);
            }
            ...
            const emotional = row.meta?.emotional_state ?? null;
            if (emotional) {
                if (emotional.trust >= 70)
                    numericTotal += 5;
                if (emotional.anger >= 70)
                    numericTotal -= 10;
                if (emotional.boredom >= 70)
                    numericTotal -= 8;
            }
            const ended = row.meta?.ended || false;
            const endReason = row.meta?.end_reason || null;
            if (ended && endReason) {
                if (endReason === "closed")
                    numericTotal += 8;
                else if (endReason === "angry")
                    numericTotal -= 20;
                else if (endReason === "bored")
                    numericTotal -= 15;
            ...
            let cfgForAward = null;
            try {
                cfgForAward = await getScoringConfig();
            }
            catch { }
            const streakNow = Number.isFinite(Number(prevMeta?.streak)) ? Number(prevMeta.streak) : 0;
            const mult = streakMultiplier(streakNow, Number.isFinite(Number(cfgForAward?.streakThreshold)) ? cfgForAward.streakThreshold : 3);
            ...
            let globalMult = 1;
            try {
                if (Number.isFinite(Number(cfgForAward?.xpMultiplier)))
                    globalMult = Number(cfgForAward.xpMultiplier);
            }
            catch { }
            ...
            const alreadyCommitted = Boolean(existingMeta?.xp_committed);
            const mergedMeta = {
                ...existingMeta,
                ...(meta || {}),
                total: numericTotal,
                ...
                xp_committed_amount: existingMeta?.xp_committed_amount ?? null,
                xp_committed_at: existingMeta?.xp_committed_at ?? null,
            };
            let finalDuration = typeof durationMs === "number" ? durationMs : row.duration_ms ?? null;
            let finalTurns = typeof turns === "number" ? turns : row.turns ?? null;
            ...
            if (finalDuration == null || finalTurns == null || !summary) {
                try {
                    ...
                    if (Array.isArray(turnRows) && turnRows.length > 0) {
                        if (finalTurns == null)
                            finalTurns = turnRows.length;
                        if (finalDuration == null && turnRows.length > 1) {
                            const first = new Date(turnRows[0].created_at).getTime();
                            const last = new Date(turnRows[turnRows.length - 1].created_at).getTime();
            ...
            const { data: updatedRow, error: fetchErr } = await supa
                .from("sparring_sessions")
                .select("id, rep_id, persona_id, difficulty, xp_awarded, total_score, created_at, duration_ms, turns, summary, flags, meta")
                .eq("id", sessionId)
                .single();
            ...
            // -----------------------------
            try {
                const repIdForAssign = String(updatedRow.rep_id || "").trim();
                const personaIdForAssign = String(updatedRow.persona_id || "").trim();
                // Accept assignmentId if explicitly provided AND looks valid
                const rawAssignmentId = typeof body?.assignmentId === "string" ? body.assignmentId.trim() : "";
                ...
                if (repIdForAssign && repIdForAssign.length > 10) {
                    ...
                    const completedCount = typeof result?.completedCount === "number"
                        ? result.completedCount
                        : 0;
                    const xpAwardedTotal = typeof result?.xpAwardedTotal === "number"
                        ? result.xpAwardedTotal
                        : 0;
                    ;
                    global.__last_completed_count = completedCount;
                    ;
                    global.__last_xp_awarded_total = xpAwardedTotal;
            ...
            const completed_count_safe = typeof global.__last_completed_count === "number"
                ? global.__last_completed_count
                : 0;
            const xp_awarded_total_safe = typeof global.__last_xp_awarded_total === "number"
                ? global.__last_xp_awarded_total
                : 0;
            return res.json({
                ok: true,
                session: updatedRow,
        ...
        if (!transcript || typeof transcript !== "string") {
            return res.status(400).json({
                ok: false,
                error: "transcript required (or provide { sessionId } to persist score)",
            });
        }
...
router.post('/sessions', express.json(), async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            const bodyRep = req.body?.repId;
            if (typeof bodyRep === 'string' && bodyRep.trim().length) {
                repId = bodyRep.trim();
            }
        }
        const DEV_REP_ID = process.env.DEV_REP_ID || '11111111-1111-1111-8111-111111111111';
        // If we still don't have a repId (e.g. local curl), fall back to dev rep
        const effectiveRepId = repId || DEV_REP_ID;
        const { personaId, difficulty, mode, targetDurationSec, assignmentId, assignment } = req.body;
        ...
        try {
            const { error: repUpsertErr } = await supa
                .from('reps')
                .upsert([{ id: effectiveRepId, name: 'Rep' }], { onConflict: 'id', ignoreDuplicates: false });
            if (repUpsertErr) {
        ...
        const payload = {
            ...
            meta: {
                ...
                buyer_style: companyProfile.buyer_style ||
                ...
                emotional_tuning: companyProfile.emotional_tuning || {
                    pressure_level: 50,
                    trust_decay: 50,
                    objection_aggression: 50,
                },
                objection_library: {
                    objection_patterns: companyProfile.objection_patterns || [],
                    competitor_names: companyProfile.competitor_names || [],
                    common_pushbacks: companyProfile.common_pushbacks || [],
                },
                persona_memory: companyProfile.persona_memory || [],
                persona_pack_version: "v1",
                dynamic_scenario: dynamicScenario,
                assignment_id: assignmentId || null,
                // game mode + target duration for time-trial / turns-based drills
                mode: mode || "standard",
                targetDurationSec: typeof targetDurationSec === "number" && Number.isFinite(targetDurationSec)
                    ? targetDurationSec
                    : null,
                ...
                emotional_state: getInitialEmotionalState(personaId || "price_sensitive", adaptiveDifficulty.effective || difficulty || "normal"),
                ...
                state: createInitialSparringState({
                    personaId: personaId || "price_sensitive",
                    difficulty: adaptiveDifficulty.effective || difficulty || "normal",
                    flagSection: assignment?.meta?.flag_section || null,
                }),
            },
        };
        const { data, error } = await supa
...
router.post('/sessions/:id/turns', express.json(), async (req, res) => {
    res.setHeader('Cache-Control', 'no-store');
    try {
        ...
        orgIdHeader =
            req.headers['x-org-id']?.toString().trim() ||
                null;
        ...
        const endedMeta = session?.meta && typeof session.meta === "object"
            ? session.meta
            : null;
        if (endedMeta && endedMeta.ended) {
            return res.status(409).json({
                ok: false,
                error: "session_ended",
                reason: endedMeta.end_reason || null,
            });
        }
        ...
        const history = (existingTurns ?? []).map((t) => ({
        ...
        const personaId = session.persona_id ||
            session?.meta?.personaId ||
            "price_sensitive";
        const difficultyVal = session.difficulty ||
            session?.meta?.difficulty ||
            "normal";
        const modeVal = session?.meta?.mode || "standard";
        ...
        const previousMeta = session?.meta && typeof session.meta === "object"
            ? session.meta
            : {};
        const prevStack = previousMeta.objection_stack || { active: [], last_added_at_turn: 0 };
        const prevEmotion = previousMeta.emotional_state ||
        ...
        const emotionalTimeline = Array.isArray(previousMeta.emotional_timeline)
        ...
        const objectionHistory = Array.isArray(previousMeta.objection_history)
        ...
        const brainState = updateSparringState(prevBrainState, {
            repText: text,
        ...
        if (hangupDecision.endNow) {
            ...
            try {
                ...
                const systemPrompt = buildPersonaSystemPrompt({
                    ...
                    difficulty: difficultyVal,
                    mode: modeVal,
                    dynamicScenario: session.meta?.dynamic_scenario || null,
                    replayContext: previousMeta.replay_context || null,
                    companyProfile: {
                        ...(previousMeta.company_persona_profile || {}),
                        objection_patterns: previousMeta.objection_library?.objection_patterns || [],
                        competitor_names: previousMeta.objection_library?.competitor_names || [],
                        common_pushbacks: previousMeta.objection_library?.common_pushbacks || [],
                        persona_memory: previousMeta.persona_memory || [],
                    },
                    buyerStyle: previousMeta.buyer_style || null,
                    emotionalState: updatedEmotion,
...
=== REP CONTEXT ===
${aiContext?.rep?.topWeaknesses?.map((w) => `- ${w.section} (${w.count})`).join("\n") || "None"}
...
`;
        ...
        // -----------------------------
        try {
            const repTurn = (insertedTurns ?? []).find((t) => t.role === "user");
            const buyerTurn = (insertedTurns ?? []).find((t) => t.role === "assistant");
            if (repTurn) {
                ...
                try {
                    const currentMeta = session?.meta && typeof session.meta === "object"
                        ? session.meta
                        : {};
                    const existing = Array.isArray(currentMeta.micro_scores)
                        ? currentMeta.micro_scores
                        : [];
                    const entry = {
                    ...
                    const existingFailedMoments = Array.isArray(currentMeta.failed_moments)
                        ? currentMeta.failed_moments
                        : [];
                    const mergedMeta = {
                        ...currentMeta,
        ...
        try {
            ...
            const mergedMeta = {
                ...
                analytics_state: {
                    ...
                    trust_drop_detected: updatedEmotion.trust < 25,
                    anger_spike_detected: updatedEmotion.anger > 75,
                    disengaged: updatedEmotion.boredom > 75,
                },
            ...
            console.warn("[sparring/turns] failed to persist emotional_state / end flags", e?.message || e);
        }
        return res.json({
            ok: true,
            turns: insertedTurns ?? [],
        ...
        console.error('POST /v1/sparring/sessions/:id/turns unexpected error', err);
        return res
            .status(400)
            .json({ ok: false, error: err?.message || 'bad_request' });
    }
...
router.post('/sessions/:id/messages', express.json(), (req, res, next) => {
...
router.post("/sessions/:id/micro-score", express.json(), async (req, res) => {
    res.setHeader("Cache-Control", "no-store");
    try {
        const sessionId = String(req.params.id || "").trim();
        if (!sessionId) {
        ...
        let lastUserText = typeof req.body?.lastUserText === "string"
            ? String(req.body.lastUserText)
            : null;
        let lastBuyerText = typeof req.body?.lastBuyerText === "string"
            ? String(req.body.lastBuyerText)
            : null;
        let repTurnId = null;
        ...
        try {
            const prevMeta = session.meta && typeof session.meta === "object"
                ? { ...session.meta }
                : {};
            let cfgForMicro = null;
            try {
            ...
            const existing = Array.isArray(prevMeta.micro_scores)
                ? prevMeta.micro_scores
                : [];
            const entry = {
                at: new Date().toISOString(),
                turn_score: micro.turn_score,
...
// -----------------------------------------
router.post("/sessions/:id/replay", express.json(), async (req, res) => {
    try {
        const sessionId = String(req.params.id || "").trim();
        ...
        const failedMoments = Array.isArray(sourceSession.meta?.failed_moments)
            ? sourceSession.meta.failed_moments
            : [];
        const targetMoment = failedMoments.find((m) => Number(m?.turn || 0) === failedTurn);
        if (!targetMoment) {
            return res.status(404).json({
                ok: false,
        ...
        const replayAttempts = Number(sourceSession.meta?.replay_attempts || 0);
...
router.get("/sessions/:id/replay", async (req, res) => {
    try {
        ...
        const failedMoments = Array.isArray(data.meta?.failed_moments)
            ? data.meta.failed_moments
            : [];
        return res.json({
            ok: true,
            session_id: sessionId,
            replay_enabled: data.meta?.replay_enabled === true,
            failed_moments: failedMoments,
        });
    }
    catch (e) {
...
router.get("/sessions", async (req, res) => {
    ...
    try {
        ...
        let query = supa
            .from("sparring_sessions")
            .select("id, rep_id, persona_id, difficulty, total_score, xp_awarded, created_at, duration_ms, turns, summary, flags, meta")
        ...
        const sessions = (data || []).map((row) => {
            ...
            const difficulty = row.difficulty ||
                row.meta?.difficulty ||
                "normal";
            const total = typeof row.total_score === "number"
                ? row.total_score
                : typeof row.meta?.total === "number"
                    ? row.meta.total
                    : null;
            const xp = typeof row.xp_awarded === "number"
                ? row.xp_awarded
                : row.meta?.xp ?? 0;
            const durationMs = typeof row.duration_ms === "number"
                ? row.duration_ms
                : row.meta?.duration_ms ?? null;
            const turns = typeof row.turns === "number"
                ? row.turns
...
router.get('/sessions/:id', async (req, res) => {
    ...
    try {
        ...
        const { data, error } = await supa
            .from('sparring_sessions')
            .select('id, rep_id, persona_id, total_score, xp_awarded, created_at, duration_ms, turns, summary, flags, meta')
            .eq('id', id)
            .single();
        if (error) {
        ...
        const personaId = data.persona_id ??
            data?.meta?.personaId ??
            'unknown';
        const total = typeof data.total_score === 'number'
            ? data.total_score
            : typeof data?.meta?.total === 'number'
                ? data.meta.total
                : null;
        const xp = typeof data.xp_awarded === 'number'
            ? data.xp_awarded
            : data?.meta?.xp ?? 0;
        const durationMs = typeof data.duration_ms === 'number'
            ? data.duration_ms
            : data?.meta?.duration_ms ?? null;
        const turnsCount = typeof data.turns === 'number'
            ? data.turns
            : Array.isArray(data?.meta?.transcript)
                ? data.meta.transcript.length
                : null;
        const summary = data.summary ?? data?.meta?.summary ?? null;
        const flags = data.flags ?? data?.meta?.flags ?? null;
        const session = {
            id: data.id,
            rep_id: data.rep_id,
            persona_id: personaId,
            difficulty: data.difficulty || data?.meta?.difficulty || "normal",
            total,
            xp_awarded: xp,
...
router.post("/analyse-turn", async (req, res) => {
    const { text } = req.body || {};
    if (!text)
        return res.json({ ok: false, error: "text_required" });
    const prompt = `
