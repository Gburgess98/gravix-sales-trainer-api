...
const clamp = (n, lo = 0, hi = 100) => Math.max(lo, Math.min(hi, Math.round(Number.isFinite(n) ? n : lo)));
...
const PRESSURE_SLOPE = {
    easy: 0.6,
    standard: 1,
    hard: 1.4,
    nightmare: 1.8,
};
export function createInitialSparringState(input) {
    const difficulty = normaliseDifficulty(input.difficulty);
    const personaId = String(input.personaId || "");
    const scepticalPersona = personaId.includes("sceptic") ||
...
export function inferStageFromText(text, previousStage) {
    if (previousStage === "ended")
        return "ended";
    const t = String(text || "").toLowerCase();
    const has = (needles) => needles.some((n) => t.includes(n));
    // Candidate stage from the rep's words
    let candidate = null;
    if (has(CLOSE_SIGNALS))
        candidate = "close";
    else if (has(PITCH_SIGNALS))
        candidate = "pitch";
    else if (has(DISCOVERY_SIGNALS))
        candidate = "discovery";
    if (!candidate)
        return previousStage === "opening" ? "discovery" : previousStage;
...
export function inferObjectionType(text) {
    const t = String(text || "").toLowerCase();
...
function sectionToObjectionType(section) {
...
const EMA_ALPHA = 0.4;
const ema = (prev, next) => clamp(prev + EMA_ALPHA * (next - prev));
export function updateSparringState(previous, turn) {
    ...
    if (score !== null) {
        ...
        if (score < 55)
            pressureLevel += Math.round(8 * slope);
        else if (score >= 70)
            pressureLevel -= Math.round(6 * slope);
    }
    ...
    const hedging = /\b(maybe|i think|probably|not sure|i guess|sort of|kind of)\b/i.test(turn.repText || "");
    const base = score ?? 50;
    const repPerformance = {
        ...
        objectionHandling: ema(prev.repPerformance.objectionHandling, prev.objectionState.active ? base : prev.repPerformance.objectionHandling),
        progression: ema(prev.repPerformance.progression, stage !== prev.stage && stage !== "ended" ? Math.max(base, 60) : base),
    };
    // Buyer mood from emotional engine + trust
    let buyerMood = prev.buyerMood;
    if (emotional) {
        if (emotional.anger > 65)
            buyerMood = "frustrated";
        else if (emotional.trust > 65)
            buyerMood = "warm";
        else if (emotional.boredom > 60 || emotional.trust < 30)
            buyerMood = "sceptical";
        else if (emotional.trust >= 45 && (score ?? 0) >= 60)
            buyerMood = "curious";
        else
            buyerMood = "neutral";
    }
    // Next buyer move (deterministic rule ladder)
    let nextBuyerMove = "ask_question";
    if (stage === "ended")
        nextBuyerMove = "close_window";
    else if (emotional && emotional.boredom > 75)
        nextBuyerMove = "close_window";
    else if (objectionState.active && !objectionState.resolved)
        nextBuyerMove = "push_back";
    else if (pressureLevel > 70)
        nextBuyerMove = "raise_objection";
    else if (score !== null && score >= 70 && trustLevel >= 55)
        nextBuyerMove = "soften";
    else if (stage === "pitch" || stage === "close")
        nextBuyerMove = "request_info";
    else
        nextBuyerMove = "ask_question";
    return clampState({
        stage,
...
// ── Prompt summary ───────────────────────────────────────────────────────────
export function summariseStateForPrompt(state) {
...
export function coerceState(raw, fallbackInput) {
    if (raw && typeof raw === "object" && raw.stage && raw.repPerformance) {
        try {
